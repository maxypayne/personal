...
export class TextareaComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  @ViewChild('areaElem', { static: true }) elArea: ElementRef;
  ...
  onPaste(event) {
    if (this.id && this.id.slice(0, 9) !== 'codeVerif') {
  ...
  handleAttr(how) {
    const {
      ...
      showRepere, etatVehicule, otherErrorMessage,
    } = this.areaAttr;
    const attr = accent ? id + 'Acc' : id;
    if (attr && attrGlobal[attr]) {
      const {
        type, description: descriptionAttr, exemple: exempleAttr, info, length, min, max, autocomplete,
  ...
  handleNativeElement(how) {
    this.elem = this.elArea ? this.elArea.nativeElement : null;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.clear && changes.clear.previousValue !== this.clear) {
      this.handleClear();
    }
  }
  handlePushData(which) {
    if (!this.id) {
      this.id = this.areaAttr.id;
      this.handleAttr(which);
    }
    this.focusData = null;
    this.handleNativeElement(which);
  }
  checkData(value, key, how?) {
    const isCheck = check.checkData(value, this.type, this.optionnel);
    if (isCheck) {
      ...
      if (status) {
        ...
        this.handleValue(cleaned, true, null, how !== 'pushDataNoEmit');
        if (how === 'pushData' || how === 'pushDataNoEmit' || how === 'pushDataRegister' || how === 'paste') {
          this.newData = cleaned;
  ...
  handleValue(cleaned, checked, error, emit) {
    if (cleaned || checked) {
      this.data = cleaned;
    } else {
  ...
  handleKeyup(key) {
    ...
    if (this.elem && !keyGhost.includes(key)) {
      this.checkData(this.elem.value, key);
    }
  }
  handleKeydown(key) {
  ...
  handleFocus() {
    if (!this.lock) {
      ...
      if (this.elem && this.eraseOnClick) {
        ...
        this.renderer.setAttribute(this.elem, 'cleanedData', null);
  ...
  handleHoverBloc(state) {
    if (state) {
      const height = this.elArea.nativeElement.offsetHeight;
  ...
  registerData(how: string) {
    if (this.idToUpdate && this.key && (this.newData || this.optionnel) && this.newData !== this.focusData && !this.fakeTrue) {
      const cleaned = this.cleanedReform ? this.cleanedReform.trim().trimLeft() : this.newData ? this.newData.trim().trimLeft() : null;
      let value;
      if (this.type === 'date') {
